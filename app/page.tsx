  ...
  <section className="mx-auto max-w-[1400px] px-3 sm:px-5 lg:px-6 py-14 sm:py-16 lg:py-18">
        ...
        >
          <div>
            <div className="relative inline-block pb-2">
              ...
              <span className="pointer-events-none absolute -bottom-0.5 left-0 right-0 h-[2px] rounded-full bg-slate-900/20 dark:bg-white/20" />
            </div>
            <p className="mt-4 text-base sm:text-lg text-slate-700 dark:text-slate-300 max-w-prose">
            ...
            <div className="mt-6 flex flex-wrap gap-3 sm:gap-3.5">
              <a href="#projects" className="rounded-xl px-5 py-2.5 bg-slate-900 text-white dark:bg-white dark:text-slate-900 shadow-sm hover:shadow focus:outline-none focus-visible:ring-2 focus-visible:ring-slate-500/60">
                View Projects
              </a>
          ...
          <div className="mx-auto w-full max-w-[380px] sm:max-w-[420px] md:max-w-[450px] lg:max-w-[480px]">
            ...
            >
              <div className="absolute inset-0 rounded-full border-2 border-slate-900/70 dark:border-white/60" />
        ...
        <div className="mt-4 md:mt-6 grid gap-5 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {cards.map((c) => (
            <motion.div
              ...
              className="group relative rounded-2xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm hover:shadow-lg"
            >
              {/* top accent (neutral) */}
              <div className="pointer-events-none absolute inset-x-0 top-0 h-[2px] rounded-t-2xl bg-slate-900/10 dark:bg-white/10" />
              <div className="p-5">
                <div className="flex items-center justify-between">
                  <div className="text-lg font-bold text-slate-900 dark:text-slate-100">
                    {c.title}
                  </div>
                  <span className="inline-flex h-7 w-7 items-center justify-center rounded-full bg-slate-900/5 dark:bg-white/10 text-slate-600 dark:text-slate-300">•</span>
                </div>
                <div className="mt-3 flex flex-wrap gap-2.5">
                  {c.techs.map((t) => (
                    <span key={t} className="inline-flex items-center gap-1 rounded-full border border-slate-300 dark:border-slate-600 px-3 py-1.5 text-sm bg-white dark:bg-slate-900">
        ...
        <div className="mt-4 md:mt-6 grid gap-5 sm:gap-6 sm:grid-cols-2 lg:grid-cols-3 items-stretch">
          {achievements.map((a) => (
            <motion.div
              ...
              whileHover={{ y: -4 }}
              transition={{ type: "spring", stiffness: 200, damping: 20 }}
              className="relative rounded-2xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-sm hover:shadow-lg"
            >
              {/* left accent bar (neutral) */}
              <div className="absolute left-0 top-0 bottom-0 w-[2px] rounded-l-2xl bg-slate-900/10 dark:bg-white/10" />
              <div className="p-5">
                <div className="flex items-start gap-3">
                  <div className="flex h-10 w-10 items-center justify-center rounded-full bg-slate-900/5 dark:bg-white/10 text-xl">
                    {a.icon ?? "🏆"}
                  </div>
      ...
      <Section id="contact" title="Contact" subtitle="Let's build something together">
        <div className="relative">
          <motion.div
            className="relative rounded-3xl border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 shadow-lg overflow-hidden"
