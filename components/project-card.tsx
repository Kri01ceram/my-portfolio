      ...
      <Card className="h-full rounded-2xl">
            ...
            <div className="flex items-center gap-2">
              {p.repo && (
                <motion.a
                  ...
                  whileTap={{ scale: 0.95 }}
                  className="inline-flex h-9 w-9 items-center justify-center rounded-full border border-input bg-card hover:bg-secondary/30 transition"
                >
        ...
        <CardFooter className="border-t">
          <div className="mt-2 flex flex-wrap gap-2.5">
            {p.tags.map((t) => (
              <motion.span key={t} whileHover={{ y: -2 }} className="inline-flex items-center gap-1 rounded-full border border-input bg-card px-2.5 py-1 text-xs text-foreground">
